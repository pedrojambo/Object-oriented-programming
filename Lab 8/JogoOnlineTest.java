...
public class JogoOnlineTest {
    ...
    @Test
    public void obterJogadoresEmOrdemAlfabetica() throws SenhaInvalidaException, UsuarioInexistenteException {
        Jogador jogadorEllis = jogo.cadastrarJogador("ellis", "1234");
        jogo.fazerLogin("ellis", "1234");
        Jogador jogadorMat = jogo.cadastrarJogador("mat", "4321");
        jogo.fazerLogin("mat", "4321");
        assertEquals("ellis", jogo.obterJogadoresEmOrdemAlfabetica().get(0).getUsername());
        assertEquals("mat", jogo.obterJogadoresEmOrdemAlfabetica().get(1).getUsername());
    }
    ...
    public void testarObterRanking() throws SenhaInvalidaException, UsuarioInexistenteException {
        ...
        Jogador jogadorMat = jogo.cadastrarJogador("mat", "4321");
        jogo.fazerLogin("mat", "4321");
        Partida match = jogo.iniciarPartida(jogadorEllis, jogadorMat);
        jogo.encerrarPartida(match,2);
        assertEquals("mat",jogo.obterRanking().get(0).getUsername());
        assertEquals("ellis",jogo.obterRanking().get(1).getUsername());
    }
}