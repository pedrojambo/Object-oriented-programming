...
public abstract class JogoDeDoisJogadores {
    ...
    public float getPercentualVitoriasJogador1() {
        float vitorias = 0;
        float partidas = 0;
        for(Partida i:this.historicoResultados) {
            partidas += 1;
            if (i.getResultado() == VITORIA_JOGADOR_1) {
                vitorias += 1;
            }
        }
        float result = (vitorias/partidas)*100;
        return result;
    }
    ...
    public float getPercentualVitoriasJogador2() {
        ...
        for(Partida i:this.historicoResultados) {
            partidas += 1;
            if (i.getResultado() == VITORIA_JOGADOR_2) {
                vitorias += 1;
            }
        }
        float result = (vitorias/partidas)*100;
    ...
    public float getPercentualEmpates() {
        ...
        for(Partida i:this.historicoResultados) {
            partidas += 1;
            if (i.getResultado() == EMPATE) {
                empates += 1;
            }
        }
        float result = (empates/partidas)*100;
        return result;
    }
